...
fun writeStash(stash: Stash, file: File) {
    val rawOutput = encodeStash(stash)
    val crc = computeCRC(rawOutput)
...
private fun DataOutput.writeItem(item: Item) {
    ...
    stream.writeCString(item.prefixId)
    stream.writeCString("suffixName")
    stream.writeCString(item.suffixId)
    stream.writeCString("relicName")
    stream.writeCString(item.relicId)
...
private fun computeCRC(byteArray: ByteArray): ByteArray {
    val crc = byteArray.fold(0L) { crc, value ->
        val crcLastByte = (crc and 0xFF).toInt()
        val unsignedValue = java.lang.Byte.toUnsignedInt(value)
        val crcTablePosition = unsignedValue xor crcLastByte
        (crc ushr 8) xor CRC32_TABLE[crcTablePosition]
    }

    val crcByteArray = ByteArray(4)
    (0..3).forEach {
        crcByteArray[it] = (crc ushr (it * 8)).toByte()
    }

    return crcByteArray
}

private val BEGIN_BLOCK = -1340212530
private val END_BLOCK = -559038242

private val CRC32_TABLE = arrayOf(
